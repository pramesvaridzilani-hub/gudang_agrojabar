...
export const updateStockRequestStatus = async (req: AuthenticatedRequest, res: Response) => {
  try {
      ...
      for (const item of itemsForCheck) {
        ...
        if (kemasanDetailList.length > 0) {
          ...
          for (const pkg of kemasanDetailList) {
            ...
            const config = produkGudang.kemasan.find((k: any) => Math.abs(Number(k.ukuranKg) - ukuranKg) < 0.01);
    ...
    // Business Logic: Deduct physical stock AND clear reservations when status transitions to DIKIRIM, TIBA, or SELESAI
    const isNowCompleted = status === 'DIKIRIM' || status === 'TIBA' || status === 'SELESAI';
    const wasCompleted = request.status === 'DIKIRIM' || request.status === 'TIBA' || request.status === 'SELESAI';
        ...
        for (const item of updatedRequest.items) {
          ...
          if (produkGudang) {
            ...
            for (const pkg of kemasanDetailList) {
              ...
              const reservedAmt = pkg.id === 'dummy'
                ? (ukuranKg > 0 ? Math.floor(Number(pkg.jumlahDireservasi || 0) / ukuranKg) : 0)
                : Number(pkg.jumlahDireservasi || 0);
              
              if (jumlahKemasanReq <= 0) continue;

              const configKemasan = produkGudang.kemasan.find((k: any) => Math.abs(Number(k.ukuranKg) - ukuranKg) < 0.01);
              ...
              // Reset child item's own reservation (pack level)
              if (pkg.id !== 'dummy') {
            ...
            // Always reset parent item's own reservation (kg level) after packaging loop
            await prisma.itemPengajuanStok.update({
              where: { id: item.id },
              data: { jumlahDireservasi: 0 }
            });
    ...
    // Business Logic: Release reservations if status transitions to DITOLAK
    const isNowRejected = status === 'DITOLAK';
    const wasRejected = request.status === 'DITOLAK';
    
    if (isNowRejected && !wasRejected) {
      try {
        for (const item of updatedRequest.items) {
          const reservedKg = item.jumlahDireservasi || 0;
          ...
          if (produkGudang) {
            // Deduct from bulk reserved
            if (reservedKg > 0) {
              await prisma.produkGudang.update({
                where: { id: produkGudang.id },
                data: {
                  stokReserved: { decrement: reservedKg }
                }
              });
            }
            ...
            for (const pkg of kemasanDetailList) {
              const reservedPacks = pkg.jumlahDireservasi || 0;
              if (reservedPacks > 0) {
                ...
                if (config) {
                  await prisma.konfigurasiKemasan.update({
                    where: { id: config.id },
                    data: {
                      stokKemasanReserved: { decrement: reservedPacks }
                    }
                  });
                }
            ...
            // Always reset parent reservation
            await prisma.itemPengajuanStok.update({
              where: { id: item.id },
              data: { jumlahDireservasi: 0 }
            });
        ...
        console.error('Failed to release warehouse reservations on reject:', (err as Error).message || err);
      }
    }
